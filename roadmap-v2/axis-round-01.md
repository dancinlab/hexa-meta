...
- `~/core/nexus/shared/bisociation/unified/ouroboros_unified.hexa` — 3-way OUROBOROS integration (cycle_tick / get_fixed_point / advance).
- `~/core/nexus/shared/harness/growth_tick.hexa` — 30-minute autonomous firing.
- `~/core/nexus/shared/harness/nexus_growth_daemon.hexa` — launchd daemon (currently STUB, MEMORY project-daemon-stub.md).
- `~/core/nexus/shared/bisociation/unified/phi_ratchet.hexa` — monotone non-decreasing Φ ratchet.
- `~/core/nexus/shared/discovery_log.jsonl` + `.sqlite` — self-evolution history bus.
- `~/core/nexus/shared/growth_bus.jsonl` — growth event bus.
- `~/core/nexus/shared/convergence/nexus.json` — evolution convergence SSOT.
...
**Existence evidence (nexus assets)**:
- `~/core/nexus/shared/n6/atlas.n6` — reality-map body.
- `~/core/nexus/shared/n6/atlas.n6.stats` / `.deg` — sidecar metadata.
- `~/core/nexus/shared/n6/atlas_health.hexa` — health check.
- `~/core/nexus/shared/n6/math_atlas.{db,dot,html,md}` — math atlas.
- `~/core/nexus/shared/n6/atlas_ossify_mk2.py` + `atlas_ws_server.py` — ossification / realtime serving.
- `~/core/nexus/docs/index.html` — 3D reality-map frontend.
